...
function JSFromUnreal(message) {
	console.log("Message received from Unreal: ", message);
	// 在这里处理从Unreal Engine接收到的消息
}
function SendToUE() {

	var inputElement = document.getElementById('shuru');
    ...
    Module.ccall('reciveMsg', 'null', ['string'], [jsonString]);
   // Module.ccall('receiveString', 'null', ['string'], [jsonString]);
}

function SendmsgToUE(msg) {
   
	var jsonString = JSON.stringify(msg); // 将JavaScript对象转换为JSON字符串
...
console.log("Emscripten version: D:/ProgramFiles/EpicGames/ue-4.27-html5-es3/Engine/Platforms/HTML5/Build/emsdk/emsdk-3.1.60/upstream/emscripten");
console.log("Emscripten configuration: D:/ProgramFiles/EpicGames/ue-4.27-html5-es3/Engine/Platforms/HTML5/Build/emsdk/emsdk-3.1.60/.emscripten");
...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_UE4HTML5JsComm', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202407110221, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = sessionStorageCommandLine != null ? [sessionStorageCommandLine] : ['../../../UE4HTML5JsComm/UE4HTML5JsComm.uproject','-stdout',];
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('UE4HTML5JsComm.wasm'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('UE4HTML5JsComm.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('UE4HTML5JsComm.data.js'));
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('UE4HTML5JsComm.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('UE4HTML5JsComm.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('UE4HTML5JsComm.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('UE4HTML5JsComm.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
